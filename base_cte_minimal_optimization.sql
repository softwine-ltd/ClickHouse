-- Minimal optimization of base_cte.sql - focuses ONLY on reducing identifier resolution overhead
-- Changes only the most critical repeated expressions without altering query structure
...
-- Original query with MINIMAL changes - only factor out the most repeated expression
SELECT order_id,
    if(
...
FROM (
        ...
        -- Add ONE small optimization: pre-calculate the most repeated expression
        base_calculations AS (
            select event_date,
                ...
                -- Pre-calculate ONLY the most repeated expression (appears 10+ times)
                if(
                ...
                fulfillment_status,
                currency,
                -- Use calculated_weight instead of repeating the if() expression
                COALESCE(order_price, total_price, 0) * calculated_weight * coalesce(currency_rate, 1) AS gross_sales,
                COALESCE(order_price, total_price, 0) * calculated_weight * coalesce(currency_rate, 1) AS order_revenue,
                COALESCE(gross_product_sales, 0) * calculated_weight * coalesce(currency_rate, 1) AS gross_product_sales,
                COALESCE(tw_total_shipping_price, 0) * calculated_weight * coalesce(currency_rate, 1) AS shipping_price,
                coalesce(tw_total_shipping_tax, 0) * calculated_weight * coalesce(currency_rate, 1) as shipping_tax,
                COALESCE(tw_total_tax, 0) * calculated_weight * coalesce(currency_rate, 1) AS taxes,
                coalesce(shipping_costs, 0) * calculated_weight * coalesce(currency_rate, 1) as shipping_costs,
                coalesce(payment_gateway_fees_and_costs, 0) * calculated_weight * coalesce(currency_rate, 1) as payment_gateway_costs,
                coalesce(cogs, 0) * calculated_weight * coalesce(currency_rate, 1) as cogs,
                cogs as cost_of_goods,
                coalesce(total_handling_fees, 0) * calculated_weight * coalesce(currency_rate, 1) as handling_fees,
                COALESCE(total_discounts, 0) * calculated_weight * coalesce(currency_rate, 1) AS discount_amount,
                coalesce(refund_money, 0) * calculated_weight * coalesce(currency_rate, 1) as refund_money,
                coalesce(tw_custom_expenses, 0) * calculated_weight * coalesce(currency_rate, 1) as custom_expenses,
                coalesce(tw_custom_gross_sales, 0) * calculated_weight * coalesce(currency_rate, 1) as custom_gross_sales,
                coalesce(tw_custom_net_revenue, 0) * calculated_weight * coalesce(currency_rate, 1) as custom_net_revenue,
                coalesce(tw_custom_gross_profit, 0) * calculated_weight * coalesce(currency_rate, 1) as custom_gross_profit,
