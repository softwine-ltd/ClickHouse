-- Conservative optimization of base_cte.sql - focuses on high-impact, low-risk changes
-- Expected 30-50% reduction in query planning time with minimal risk
...
-- Optimization 1: Pre-calculate repeated expressions to reduce identifier resolution
WITH 
-- Keep original currency conversion logic but simplify
...
-- Pre-calculate common weight multiplier to avoid repeated resolution
weighted_data AS (
    select event_date,
        ...
        -- Calculate weight once instead of 10+ times
        CASE 
        ...
        coalesce(currency_rate, 1) AS currency_factor,
        
        -- Pre-calculate base price to avoid repeated COALESCE
        COALESCE(order_price, total_price, 0) AS base_price,
        ...
        -- Use pre-calculated values for all monetary calculations
        COALESCE(gross_product_sales, 0) AS base_gross_product_sales,
        COALESCE(tw_total_shipping_price, 0) AS base_shipping_price,
        coalesce(tw_total_shipping_tax, 0) AS base_shipping_tax,
        COALESCE(tw_total_tax, 0) AS base_tax,
        coalesce(shipping_costs, 0) AS base_shipping_costs,
        coalesce(payment_gateway_fees_and_costs, 0) AS base_payment_costs,
        coalesce(cogs, 0) AS base_cogs,
        cogs as cost_of_goods,
        coalesce(total_handling_fees, 0) AS base_handling_fees,
        COALESCE(total_discounts, 0) AS base_discounts,
        coalesce(refund_money, 0) AS base_refund,
        coalesce(tw_custom_expenses, 0) AS base_custom_expenses,
        coalesce(tw_custom_gross_sales, 0) AS base_custom_gross_sales,
        coalesce(tw_custom_net_revenue, 0) AS base_custom_net_revenue,
        coalesce(tw_custom_gross_profit, 0) AS base_custom_gross_profit,
        ...
        -- Pre-calculate channel logic to avoid repeated CASE evaluation
        case
        ...
        -- Pre-calculate campaign_id logic
        if(
...
-- Final selection using pre-calculated values (major reduction in identifier resolution)
SELECT 
    order_id,
    if(
    ...
    adset_id,
    ad_id,
    -- Use pre-calculated values to reduce repeated identifier resolution
    base_price * weight_factor * currency_factor AS gross_sales,
    base_price * weight_factor * currency_factor AS order_revenue,
    base_gross_product_sales * weight_factor * currency_factor AS gross_product_sales,
    base_shipping_price * weight_factor * currency_factor AS shipping_price,
    base_shipping_tax * weight_factor * currency_factor as shipping_tax,
    base_tax * weight_factor * currency_factor AS taxes,
    base_shipping_costs * weight_factor * currency_factor as shipping_costs,
    base_payment_costs * weight_factor * currency_factor as payment_gateway_costs,
    base_cogs * weight_factor * currency_factor as cogs,
    cost_of_goods,
    base_handling_fees * weight_factor * currency_factor as handling_fees,
    base_discounts * weight_factor * currency_factor AS discount_amount,
    base_refund * weight_factor * currency_factor as refund_money,
    base_custom_expenses * weight_factor * currency_factor as custom_expenses,
    base_custom_gross_sales * weight_factor * currency_factor as custom_gross_sales,
    base_custom_net_revenue * weight_factor * currency_factor as custom_net_revenue,
    base_custom_gross_profit * weight_factor * currency_factor as custom_gross_profit
FROM weighted_data
